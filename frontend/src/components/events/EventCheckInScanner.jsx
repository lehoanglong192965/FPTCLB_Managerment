...
function ResultBanner({ result }) {
  ...
  return (
    <div className={`mt-4 flex items-center gap-3 rounded-xl border p-4 ${result.success ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-red-200 bg-red-50 text-red-700"}`} role={result.success ? "status" : "alert"}>
      ...
      <div><p className="font-semibold">{result.success ? "Check-in completed" : "Check-in failed"}</p><p className="text-sm">{result.success ? result.name : result.message}</p></div>
    </div>
  );
}
...
export default function EventCheckInScanner({ eventId, sessionId, sessionStatus }) {
  ...
  if (sessionStatus !== "OPEN") {
    return <div className="mt-4 rounded-xl border border-yellow-200 bg-yellow-50 p-5 text-center text-sm text-yellow-700">This attendance session is closed. Open it before checking participants in.</div>;
  }

  const showSuccess = (payload, fallbackName) => {
    setResult({ success: true, name: payload?.participantName ?? payload?.fullName ?? fallbackName ?? "Participant", studentId: payload?.studentId });
  };
  ...
  const handleManualCheckIn = async (participant) => {
    ...
    try {
      ...
      setResult({ success: false, message: error?.response?.data?.message ?? "Unable to check this participant in." });
    } finally {
      setCheckingKey(null);
    }
  };
  ...
  const handleQrTicketCheckIn = async (ticketCode) => {
    ...
    try {
      ...
      const message = error?.response?.data?.message ?? "Unable to check this ticket in.";
    ...
    <div className="space-y-5">
      {summary && (
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-xl bg-blue-50 p-4 text-center"><p className="text-2xl font-bold text-blue-700">{summary.totalRegistered ?? "-"}</p><p className="mt-1 text-xs text-blue-600">Registered</p></div>
          <div className="rounded-xl bg-green-50 p-4 text-center"><p className="text-2xl font-bold text-green-700">{summary.totalCheckedIn ?? 0}</p><p className="mt-1 text-xs text-green-600">Checked in</p></div>
          <div className="rounded-xl bg-gray-50 p-4 text-center"><p className="text-2xl font-bold text-gray-700">{summary.totalAbsent ?? "-"}</p><p className="mt-1 text-xs text-gray-500">Absent</p></div>
        </div>
      )}

      <section className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
        <div className="mb-5 flex gap-2 border-b border-gray-200">
          <button type="button" onClick={() => { setMode("qr"); setResult(null); }} className={`border-b-2 px-3 py-2 text-sm font-semibold transition ${mode === "qr" ? "border-blue-600 text-blue-700" : "border-transparent text-gray-500 hover:text-blue-700"}`}>Scan QR ticket</button>
          <button type="button" onClick={() => { setMode("manual"); setResult(null); }} className={`border-b-2 px-3 py-2 text-sm font-semibold transition ${mode === "manual" ? "border-blue-600 text-blue-700" : "border-transparent text-gray-500 hover:text-blue-700"}`}>Manual lookup</button>
        </div>
        ...
        {mode === "qr" ? (
          ...
          <>
            <h3 className="border-b pb-3 text-lg font-bold text-gray-800">Manual check-in</h3>
            <p className="mb-5 mt-2 text-sm text-gray-500">Search by name, student ID, or phone suffix.</p>
            <div className="relative mx-auto max-w-2xl">
              <Search size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
              <input type="text" autoFocus value={query} onChange={(event) => { setQuery(event.target.value); setResult(null); }} placeholder="Search participants" className="w-full rounded-xl border border-gray-200 py-3 pl-10 pr-4 text-base outline-none shadow-sm focus:ring-2 focus:ring-blue-400" />
                ...
                {participants.map((participant) => {
                  ...
                  return (
                    <div key={participantKey} className="flex items-center justify-between px-4 py-3 hover:bg-gray-50">
                      ...
                      {alreadyCheckedIn ? <span className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-600">Already checked in</span> : <button type="button" onClick={() => { void handleManualCheckIn(participant); }} disabled={checkingKey === participantKey} className="rounded-lg bg-blue-600 px-4 py-1.5 text-xs font-semibold text-white transition hover:bg-blue-700 disabled:opacity-50">{checkingKey === participantKey ? "..." : "Check in"}</button>}
                    </div>
                  );
                })}
              </div>
            )}
            {query.trim() && !searching && participants.length === 0 && <p className="mt-4 text-center text-sm text-gray-400">No matching registration found.</p>}
      ...
      <section className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
        <div className="mb-4 flex items-center justify-between border-b pb-3"><h3 className="flex items-center gap-2 text-base font-bold text-gray-800"><Users size={18} className="text-blue-600" /> Checked-in participants</h3><span className="rounded-full bg-blue-50 px-3 py-1 text-sm font-semibold text-blue-600">{checkedIn.length}</span></div>
        <div className="relative mb-4 max-w-xs"><Search size={14} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" /><input value={listSearch} onChange={(event) => setListSearch(event.target.value)} placeholder="Filter checked-in list" className="w-full rounded-lg border border-gray-200 bg-gray-50 py-2 pl-8 pr-3 text-sm outline-none focus:border-blue-400 focus:bg-white" /></div>
        {visibleCheckedIn.length === 0 ? <p className="py-8 text-center text-sm text-gray-400">No checked-in participants.</p> : (
          <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="bg-gray-50 text-left text-gray-500"><th className="px-4 py-2">#</th><th className="px-4 py-2">Name</th><th className="px-4 py-2">Student ID</th></tr></thead><tbody>{visibleCheckedIn.map((record, index) => <tr key={record.participantKey ?? record.recordId ?? index} className="border-t border-gray-100"><td className="px-4 py-3 text-gray-400">{index + 1}</td><td className="px-4 py-3 font-medium text-gray-800">{record.fullName ?? record.name ?? "-"}</td><td className="px-4 py-3 font-mono text-gray-600">{record.studentId ?? "-"}</td></tr>)}</tbody></table></div>
        )}
      </section>
    </div>
  );
