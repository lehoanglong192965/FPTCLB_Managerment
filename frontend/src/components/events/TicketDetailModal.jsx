...
function buildVietQrUrl(amount, paymentReference) {
        ...
        <div className="space-y-4 p-5">
          ...
          {ticket.paymentStatus === "PENDING" && (
            <div className="mb-4 rounded-xl border border-orange-200 bg-orange-50 p-4">
              ...
              <div className="mb-3 rounded-lg border border-orange-200 bg-white p-3 text-center">
                <img
                  src={buildVietQrUrl(ticket.amountDue, ticket.paymentReference)}
                ...
                <div className="mt-2 text-left text-xs text-slate-700">
                  <p className="m-0"><b>Ngân hàng:</b> {PAYMENT_BANK.name}</p>
                  <p className="m-0"><b>Số tài khoản:</b> {PAYMENT_BANK.accountNumber}</p>
                  <p className="m-0"><b>Chủ tài khoản:</b> {PAYMENT_BANK.accountName}</p>
                  <p className="m-0 break-all"><b>Nội dung:</b> {ticket.paymentReference}</p>
                </div>
              </div>
          ...
          {ticket.paymentStatus === "AWAITING_VERIFICATION" && (
            <div className="mb-4 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800">
              <p className="mb-1 font-bold">Đang chờ xác minh chuyển khoản</p>
              <p className="m-0">Vé QR sẽ xuất hiện sau khi SePay hoặc ban tổ chức đối soát giao dịch thành công.</p>
            </div>
          )}
