...
import { Bell, CalendarDays, Clock, Send, CheckCircle2, ShieldCheck, AlarmClock, X } from "lucide-react";
...
const NOTIF_PATH = {
  ...
  CORE_TEAM:    "/core-team/notifications",
  CLUB_MANAGER: "/manager/notifications",
  ICPDP:        "/icpdp/notifications",
};

const TYPE_META = {
  event:    { Icon: CalendarDays, iconBg: "bg-blue-50",   iconColor: "text-blue-600"   },
  deadline: { Icon: Clock,        iconBg: "bg-rose-50",   iconColor: "text-rose-600"   },
  recruit:  { Icon: Send,         iconBg: "bg-orange-50", iconColor: "text-orange-500" },
  approval: { Icon: ShieldCheck,  iconBg: "bg-green-50",  iconColor: "text-green-600"  },
  reminder: { Icon: AlarmClock,   iconBg: "bg-amber-50",  iconColor: "text-amber-500"  },
  general:  { Icon: Bell,         iconBg: "bg-purple-50", iconColor: "text-purple-600" },
};
...
function resolveUserInfo(authUser) {
  ...
  const role    = authUser?.role || (TokenService.getRole() !== "GUEST" ? TokenService.getRole() : null) || "MEMBER";
  return { name, role };
}
...
export default function Topbar() {
  ...
  return (
    <header className="h-[58px] bg-white border-b border-[#F0F0F0] flex items-center justify-end px-7 sticky top-0 z-50 shrink-0 gap-4">
      {/* Right side */}
      <div className="flex items-center gap-2">
        {/* Bell */}
