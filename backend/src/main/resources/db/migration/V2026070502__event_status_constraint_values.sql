...
WHERE cc.parent_object_id = OBJECT_ID(N'dbo.Event')
  AND cc.name IN (N'CK_EventStatus', N'CK_Event_Status');
...
IF NOT EXISTS (
    ...
    FROM sys.check_constraints cc
    WHERE cc.parent_object_id = OBJECT_ID(N'dbo.Event')
      AND cc.name = N'CK_EventStatus'
)
BEGIN
    ALTER TABLE dbo.Event WITH CHECK ADD CONSTRAINT CK_EventStatus CHECK (
        eventStatus IN (
            'Draft', 'Pending', 'PendingApproval', 'Approved', 'Rejected', 'Cancelled', 'Canceled',
            ...
            'DRAFT', 'PENDING', 'PENDING_APPROVAL', 'PENDINGAPPROVAL', 'APPROVED', 'REJECTED',
